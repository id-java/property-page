  ...
  function (window) {
    ...
    var Plugin = function (opts) {
      ...
      publicAPIs.init = function (options) {
        // Feature test
        if (!supports())
          throw "ysSlider: This browser does not support the required JavaScript methods and browser APIs.";
        ...
        // Select wrapper
        settings.wrapper =
          typeof settings.wrapper === "string"
            ? document.querySelector(settings.wrapper)
            : settings.wrapper;

        // Check if a valid element
        if (!isElement(settings.wrapper))
          throw new TypeError(
            "ysSlider: Please select a valid slider wrapper."
          );
        ...
        if (settings.controls === true) {
          // Select controlBar
          settings.controlBar =
            typeof settings.controlBar === "string"
              ? document.querySelector(settings.controlBar)
              : settings.controlBar;

          // Check if a valid element
          if (!isElement(settings.controlBar))
            throw new TypeError(
              "ysSlider: Please select a valid slider control element."
            );
